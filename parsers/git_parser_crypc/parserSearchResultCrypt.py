...
collection.create_index(f'bitcoin_adress', unique=True) # обозначаем поле, чтобы адрес кошелька не повторялся


def parsePage(start=1, query="crypto", limit=100):
...
# запускаем парсер
'''
query: по какой фразе ищем репозитории. по умолчанию "crypto"
